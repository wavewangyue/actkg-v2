...
def automata(seg_list):
    ...
    if all(state['path'] == [] for state in states):
        relation_p = None
        for state in states:
            if (state['header'] is not None) and (state['available_words'] != []):
                source = {'name': state['header']['name'], 'label': state['header']['label'], 'neoId': state['header']['neoId']}
                ...
                # 对于所有关系属性逐个进行相似度匹配, 大于阈值就进行状态转移
                for link in caches[source['neoId']]:
                    score = serviceWord2vec.get_similarity(list(jieba.cut(relation_p)), list(jieba.cut(link['name'])))
                    if score > threshold_1:
                        new_path = [step for step in state['path']]
                        target = {'name': link['target_name'], 'label': link['target_label'], 'neoId': link['target_neoId']}
                        new_path.append([source, link['name'], target])
                        new_score = state['score'] * (1 + score - threshold_1)
                        states.append({'header': state['header'], 'tailer': target, 'available_words': [], 'path': new_path, 'score': new_score})
    ...
    if all(state['path'] == [] for state in states):
        for state in states:
            if (state['header'] is not None) and (state['available_words'] != []):
                ...
                res = owlSubServers.answer_selection(str(''.join(seg_list)), str(description))
                if res is None:
                    break
                answer = res['answer']
                point = float(res['point'])
                if point > threshold_3:
                    abstract = answer if len(answer) < 10 else answer[:8]+'...'
                    new_path = [step for step in state['path']]
                    source = {'name': state['header']['name'], 'label': state['header']['label'], 'neoId': state['header']['neoId']}
                    target = {'name': abstract, 'label': '实体描述文本', 'neoId': None, 'ans_from_desc':answer}
